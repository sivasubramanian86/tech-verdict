export class ComparisonUI {
  ...
  private setupEventListeners(): void {
    const optionInput = document.getElementById('optionInput') as HTMLInputElement;
    if (optionInput) {
  ...
  async compare(): Promise<void> {
    const constraints = (document.getElementById('constraints') as HTMLTextAreaElement).value.trim();
    ...
    try {
      ...
      resultsDiv!.innerHTML = `<div class="error">Error: ${(error as Error).message}</div>`;
    }
  }
      ...
      <div class="decision-section">
        ...
        ${data.clarifyingQuestions.length > 0 ? `
          <div class="clarifying-questions">
            ...
            ${data.clarifyingQuestions.map((q: string) => `<div class="question">• ${q}</div>`).join('')}
          </div>
        ` : ''}
      </div>
      <div class="results">
    ...
    data.options.forEach((option: any) => {
      html += `
        <div class="option-card">
          ...
          <div class="scores">
            ${Object.entries(option.scores).slice(0, 5).map(([key, value]: [string, any]) => `
